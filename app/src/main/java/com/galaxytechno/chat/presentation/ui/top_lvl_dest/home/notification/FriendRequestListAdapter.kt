...
class FriendRequestListAdapter(context: Context, val delegate: FriRequestListDelegate) :
    BaseRecyclerAdapter<FriRequestListViewHolder, FriReqVos>(context) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<FriReqVos> {

        return if (viewType == VIEW_TYPE_ITEM) {
            val v = mLayoutInflator.inflate(R.layout.layout_fri_request_confirmation, parent, false)
            FriRequestListViewHolder(v, delegate)
        } else {
    ...
    override fun onBindViewHolder(holder: BaseViewHolder<FriReqVos>, position: Int) {
        if (holder is FriRequestListViewHolder) {
...
//        Handler().postDelayed({ this.isLoading = false }, 5000)
    ...
    class LoadMoreViewHolder(v: View) : BaseViewHolder<FriReqVos>(v) {
