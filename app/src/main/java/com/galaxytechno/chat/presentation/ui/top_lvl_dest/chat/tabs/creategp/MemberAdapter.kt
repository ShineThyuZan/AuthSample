...
class MemberAdapter(
    context: Context,
    private var selectedList: List<FriListVos>,
    val onClickedChecked: (item: FriListVos, isChecked: Boolean) -> Unit
) :
    BaseRecyclerAdapter<SelectedFriListViewHolder, FriListVos>(context) {
    ...
    var _selectedList: List<FriListVos> = listOf()
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<FriListVos> {
        ...
        return if (viewType == VIEW_TYPE_ITEM) {
            val v = mLayoutInflator.inflate(R.layout.layout_selected_friend_list, parent, false)
            SelectedFriListViewHolder(v, onClickedChecked)
        } else {
    ...
    override fun onBindViewHolder(holder: BaseViewHolder<FriListVos>, position: Int) {
        if (holder is SelectedFriListViewHolder) {
            super.onBindViewHolder(holder, position)
//            val isSelected = selectedList.contains(items[position])
//            holder.ivSelected?.isChecked = false
            holder.ivSelected?.setOnCheckedChangeListener { buttonView, isChecked ->
                Timber.tag("tp.isssss").d(isChecked.toString())
                onClickedChecked(mData!![position], isChecked)
...
//            delegate.loadMore()
    ...
    fun updateSelectedList(list: List<FriListVos>) {
        _selectedList = list
        // notifyDataSetChanged()
    }
