...
import com.galaxytechno.chat.databinding.FragmentChatSearchBinding
...
@AndroidEntryPoint
class ChatSearchFragment :
    OtherLvlFragment<FragmentChatSearchBinding>(FragmentChatSearchBinding::inflate),
    ChatRecentMsgDelegate {

    private val viewModel: ChatViewModel by viewModels()
    private lateinit var chatRecentMsgAdapter: ChatRecentMsgAdapter
    private var msgRecentList: MutableList<UserVos> = mutableListOf()
    ...
    private fun setUpCountryListRecyclerView() {
        chatRecentMsgAdapter = ChatRecentMsgAdapter(requireContext(), this@ChatSearchFragment)
            ...
            when (it) {
                is RemoteEvent.SuccessEvent -> {
                    ...
                    * setUpCountryListRecyclerView()
                    if (it.data?.data!!.countryList.isEmpty()) {
                        binding.offlineState.constOfflineState.visibility = View.VISIBLE
                        binding.constRecent.visibility = View.INVISIBLE
                    } else {
