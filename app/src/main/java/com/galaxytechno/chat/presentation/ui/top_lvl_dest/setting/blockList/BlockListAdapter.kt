...
import com.galaxytechno.chat.presentation.ui.top_lvl_dest.home.notification.FriRequestListDelegate
import com.galaxytechno.chat.presentation.ui.top_lvl_dest.home.notification.FriRequestListViewHolder

class BlockListAdapter(context: Context, val delegate: BlockListDelegate) :
    BaseRecyclerAdapter<BlockListViewHolder, FriListVos>(context) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseViewHolder<FriListVos> {
        return if (viewType == VIEW_TYPE_ITEM) {
            val v = mLayoutInflator.inflate(R.layout.layout_block_list_view_pod, parent, false)
            BlockListViewHolder(v, delegate)
        } else {
    ...
    override fun onBindViewHolder(holder: BaseViewHolder<FriListVos>, position: Int) {
        if (holder is BlockListViewHolder) {
